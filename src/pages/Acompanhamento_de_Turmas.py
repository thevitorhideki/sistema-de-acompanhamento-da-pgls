...
def fetch_data():
    ...
    filtered_teachers = teachers[~teachers['departmentName'].isin(
        excluded_departments)]
...
def group_responses(filtered_teachers, responses, surveys_dim, question_dim, response_set_dim, period_dim, course_dim):
    ...
    responses_joined_with_teachers = pd.merge(
        responses, filtered_teachers, left_on='personAssesseeId', right_on='personId')
...
st.title("Acompanhe o desempenho das turmas")
...
responses_grouped = group_responses(
    filtered_teachers, responses, surveys_dim, question_dim, response_set_dim, period_dim, course_dim)
...
if professor != 'Todos':
    ...
    st.write(dados_filtrados[[
        'departmentName', 'courseName', 'turma', 'classCode']].drop_duplicates())
    ...
    teachers_username = filtered_teachers[filtered_teachers['fullName'].str.upper(
